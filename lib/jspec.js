...
// TODO: JSpec rename to something better
var Jspec = {
	...
	matchers : {
		...
		be_at_least     : '>=',
		be_at_most      : '<=',
		match           : 'got.match(expected)',
		be_empty        : 'got.length == 0',
		be_true         : 'got == true',
		be_false        : 'got == false',
		have_length_of  : 'got.length == expected',
		respond_to      : 'typeof got[expected] == "function"',
	},
	...
	Matcher : function (name, matcher, expected, got, negate) {
		...
		if (typeof matcher == 'string') {
		  if (matcher.length < 4) body = 'got ' + matcher + ' expected'
		  else body = matcher
		  matcher = { match : function(expected, got) { return eval(body) } }
	  }
		
		function generateMessage() {
		  return 'expected ' + got + (negate ? ' not' : '') + ' to ' + name.replace(/_/g, ' ') + ' ' + expected
		}
