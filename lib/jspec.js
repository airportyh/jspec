...
var Jspec = {
	...
	matchers : {
		equal : {
			match : function(expected, got) {
				return got == expected
			},

			fail : function(expected, got, negate) { 
				return 'expected ' + got + ' to ' + (negate ? 'not ' : '') + 'equal ' + expected
			},

			pass : function(expected, got, negate) {
				return 'expected ' + expected + ' to ' + (negate? 'does not equal' : 'equal') + ' ' + expected
			}
		}
	},
	...
	Matcher : function (matcher, expected, got, negate) {
		var self = this
		this.message = ''
		this.passed = false
    
		function message(type) {
			self.message = matcher[type](expected, got, negate)
		}
		...
		this.match = function() {
			return matcher.match.call(got, expected, got.valueOf())
		}
    ...
    if (!report) throw 'JSpec requires div#jspec to output its reports'
		...
		markup += '<span class="passes">Passes: <em>' + results.stats.passes + '</em></span> '
		markup += '<span class="failures">Failures: <em>' + results.stats.failures + '</em></span>'
		markup += '</div>'
		...
		results.each(results.suites, function(description, suite){
		  ...
		  results.each(suite.specs, function(i, spec){
		    ...
		    if (spec.passed()) markup += '<p class="pass">' + spec.description + '</p>'
		    else               markup += '<p class="fail">' + spec.description + '</p>' 
		  })
		})
	...
	Spec : function(description, body) {
	  // TODO: fix assertions, duplicating twice for some reason
		this.body = body
		this.description = description
		this.assertions = []
	...
	defineMatchers : function(matchers) {
		this.each(matchers, function(key, callbacks){
			Object.prototype['should_' + key] = function(other) {
			  var matcher = new Jspec.Matcher(matchers[key], this, other)
				Jspec.currentSpec.assertions.push(matcher.exec())
			}
			Object.prototype['should_not_' + key] = function(other) {
			  var matcher = new Jspec.Matcher(matchers[key], this, other, true)
				Jspec.currentSpec.assertions.push(matcher.exec())
			}
		})
		return this
