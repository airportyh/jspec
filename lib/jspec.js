...
var JSpec = {
  ...
  evalHook : function(suite, hook) {
    console.log(eval(suite.hooks[hook]));
    if (suite.hooks[hook]) this.evalBody(suite.hooks[hook], "Error in hook '" + hook + "', suite '" + suite.description + "'")
  },
  
  evalBody : function(string, errorMessage) {
    try { eval(string) }
