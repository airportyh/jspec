...
var JSpec={version:'0.1.1',suites:{},stats:{specs:0,assertions:0,failures:0,passes:0},matchers:{eql:'==',be:'alias eql',equal:'===',be_greater_than:'>',be_less_than:'<',be_at_least:'>=',be_at_most:'<=',be_a:'actual.constructor == expected',be_an:'alias be_a',be_empty:'actual.length == 0',be_true:'actual == true',be_false:'actual == false',be_type:'typeof actual == expected',match:'actual.match(expected)',have_length_of:'actual.length == expected',respond_to:'typeof actual[expected] == "function"',include:{match:function(expected,actual){if(actual.constructor==String)return actual.match(expected)
else return expected in actual}},throw_error:{match:function(expected,actual){try{actual()}
catch(e){return true}}}},Matcher:function(name,matcher,expected,actual,negate){var self=this
this.name=name
this.message=''
this.passed=false
...
function generateMessage(){return'expected '+actual+' to '+(negate?' not ':'')+name.replace(/_/g,' ')+' '+expected}
...
this.passes=function(){return negate?!this.match():this.match()}
this.exec=function(){if(this.passes())pass()
else fail()
return this}},Formatter:function(results){var markup='',report=document.getElementById('jspec')
...
results.each(results.suites,function(description,suite){if(suite.ran){markup+='<h2>'+description+'</h2>'
...
markup+='</div></div>'
report.innerHTML=markup},Suite:function(description){this.specs=[],this.hooks={}
this.description=description
this.ran=false
this.addSpec=function(spec){this.specs.push(spec)
spec.suite=this}
this.hook=function(hook){if(body=this.hooks[hook])
JSpec.evalBody(body,"Error in hook '"+hook+"', suite '"+this.description+"'")}},Spec:function(description,body){this.body=body
this.description=description
this.assertions=[]
...
this.requiresImplementation=function(){return this.assertions.length==0}},each:function(object,callback){for(var key in object){if(typeof object[key]=='function')continue
if(callback.length==1)
callback.call(this,object[key])
else
callback.call(this,key,object[key])}
return this},addMatchers:function(matchers){this.each(matchers,function(key,callbacks){Object.prototype['should_'+key]=function(other){var matcher=new JSpec.Matcher(key,matchers[key],other,this)
JSpec.currentSpec.assertions.push(matcher.exec())}
Object.prototype['should_not_'+key]=function(other){var matcher=new JSpec.Matcher(key,matchers[key],other,this,true)
JSpec.currentSpec.assertions.push(matcher.exec())}})
return this},report:function(){var formatter=new JSpec.Formatter(this)
...
suite.hook('after')
return this},evalBody:function(body,errorMessage){try{eval(this.preProcessBody(body))}
catch(e){throw(errorMessage||'Error: ')+e}},preProcessBody:function(body){body=body.replace(/\.should_(\w+)(?: |$)(.*)$/gm,'.should_$1($2)')
return body},eval:function(input){return this.parse(input)},parse:function(input){var describing,specing,capturing
...
return input.match(regexp)},load:function(file){var request=new XMLHttpRequest
request.open('GET',file,false)
request.send(null)
if(request.readyState==4)
return request.responseText},exec:function(file){this.eval(this.load(file)).run().report()}}
